...
void block_prefill_cache(float* out, const float* x, const int* input_ids, const BlockWeights& w,
    ...
    CU(cudaMalloc(&x1,(size_t)bs*d*4)); CU(cudaMalloc(&post,(size_t)bs*hc*4)); CU(cudaMalloc(&comb,(size_t)bs*hc*hc*4));
    CU(cudaMalloc(&sub,(size_t)bs*d*4)); CU(cudaMalloc(&res2,(size_t)bs*hc*d*4));
...
void block_decode_step(float* out, const float* x, const int* input_ids, const BlockWeights& w,
    ...
    float *x1,*post,*comb,*sub,*res2;
    CU(cudaMalloc(&x1,(size_t)d*4)); CU(cudaMalloc(&post,(size_t)hc*4)); CU(cudaMalloc(&comb,(size_t)hc*hc*4));
    CU(cudaMalloc(&sub,(size_t)d*4)); CU(cudaMalloc(&res2,(size_t)hc*d*4));
