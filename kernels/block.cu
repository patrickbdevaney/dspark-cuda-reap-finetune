// block.cu — full Block forward. Composition of gated pieces (hc, mla_forward, moe_forward, rmsnorm).
#include "block.h"
#include "hc.h"
...
void block_forward(float* out, const float* x, const int* input_ids, const BlockWeights& w,
                   int s, int iters, float eps, cudaStream_t stream) {
    ...
    CU(cudaMalloc(&sub,  (size_t)bs * d * 4));            // attn / moe output [bs,d]
    CU(cudaMalloc(&res2, (size_t)bs * hc * d * 4));       // attn-block output [bs,hc,d]
